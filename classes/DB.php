<?php
	...
	abstract class DB
	{
		...
		public static function getAllActor()
		{
			...
			$stmt = $connection->prepare("
				SELECT a.id, a.nombre, a.apellido a.edad AS 'movie_id'
				FROM actor as a
				ORDER BY a.nombre;
			");
			...
			$actor = $stmt->fetchAll(PDO::FETCH_ASSOC);

			$actorObject = [];

			foreach ($actors as $actor) {
				$actormovie = new actor($actor['nombre'], $actor['apellido'], $movie['peliculas']);

             $actormovie->setID($actor['id']);

				$actorObject[] = $actormovie;
			}
